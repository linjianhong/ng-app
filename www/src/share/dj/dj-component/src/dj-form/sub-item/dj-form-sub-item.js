...
!(function (window, angular, undefined) {
  ...
  function initDropdownList(param, $http, $q) {
    ...
    if (angular.isString(param.list)) {
      return $http.post(`下拉列表-${param.list}`).then(json => {
  ...
  var theTemplates = {
    /** 几个通用显示(未格式化) */
    "initValue-show": `
      <div class="flex flex-left {{$ctrl.configs.css.hostBodyShow}}">
        <span class="a">{{$ctrl.configs.title}}</span>
        <span class="b dj">{{$ctrl.initValue}}</span>
      </div>`,
    "text-show": `
      <div class="flex flex-left {{$ctrl.configs.css.hostBodyShow}}">
        <span class="a">{{$ctrl.configs.title}}</span>
        <span class="b dj">{{text}}</span>
      </div>`,
    "textarea-show": `
      <div class="flex flex-left {{$ctrl.configs.css.hostBodyShow}}">
        <span class="a">{{$ctrl.configs.title}}</span>
        <span class="b dj" br-text="{{$ctrl.initValue}}"></span>
      </div>`,
    ...
    "initValue-format-show": `
      <div class="flex flex-left {{$ctrl.configs.css.hostBodyShow}}">
        <span class="a">{{$ctrl.configs.title}}</span>
        <span class="b dj">{{$ctrl.initValue|formFormat:($ctrl.configs.show.format)}}</span>
      </div>`,
    "text-format-show": `
      <div class="flex flex-left {{$ctrl.configs.css.hostBodyShow}}">
        <span class="a">{{$ctrl.configs.title}}</span>
        <span class="b dj">{{text|formFormat:($ctrl.configs.show.format)}}</span>
      </div>`,

    /** 文本框 */
    "input": `
      <div class="a flex prompt-top" dj-form-default-tip></div>
      <djui-input class="b dj flex"
    ...
    "dropdown": `
      <div class="a flex prompt-top" dj-form-default-tip></div>
      <div class="b dj inputs flex flex-v-center">
    ...
    "tags": `
      <div class="a flex prompt-top" dj-form-default-tip></div>
      <djui-tags class="b dj item-body"
        list="list"
        init-value="value"
        on-change="$ctrl.onChange({value: value})"
      ></djui-tags>
    ...
    "tags-show": `
      <div class="flex flex-left {{$ctrl.configs.css.hostBodyShow}}">
        <span class="a">{{$ctrl.configs.title}}</span>
        <djui-tags-show class="b dj item-body" list="value"></djui-tags-show>
      </div>
      `,
  ...
  theModule
    ...
    .directive(directiveNormalize('dj-form-default-tip-mini'), function () {
      return {
        restrict: 'A',
        template: `
          <div class="required">{{$ctrl.theValid.required && '*' || ''}}</div>
