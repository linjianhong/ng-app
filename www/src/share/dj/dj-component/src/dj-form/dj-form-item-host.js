  ...
  function ctrlHostEdit($scope, $element, $timeout, $q, $compile, DjFormDefaultDefine) {
      ...
      if (componentName) {
        var template = `
          <div class="${css} {{theValid.dirty&&'ng-dirty'||''}} {{!theValid.valid&&'ng-invalid'||''}}">
            ...
            <${componentName} class="${configs.css.dataEdit || configs.css.data || 'b user flex-v-center'}"
              mode="edit"
    ...
    var theValid = $scope.theValid = this.theValid = {
      ...
      calc: () => {
        ...
        var invalid = angular.extend({ required: "required" }, theValid.configs.errortext, theValid.configs.invalid, theValid.configs.param.invalid);
        ...
        theValid.valid = true;
        theValid.tip = "";
        theValid.required = valid.required || valid.require;
        /** 开始验证 */
        if (valid.required) {
    ...
    this.onChangesCtrl = (changes) => {
      if (changes.initValue) {
        if(theValid.setValue(changes.initValue.currentValue)){
          $scope.valid = '----';
  ...
  function ctrlHostShow($scope, $element, $timeout, $q, $compile, DjFormDefaultDefine) {
    ...
    var compileConfigs = (configs, value) => {
      ...
      if (componentName) {
        var template = `
          <div class="${css}">
            <div class="flex flex-left {{configs.css.hostBodyShow}}">
              <span class="a" ng-hide="configs.hideTip">{{configs.title}}</span>
              <${componentName} class="b ${configs.css.dataShow || configs.css.data || 'flex-v-center'}"
                mode="show"
                configs="$ctrl.configs"
