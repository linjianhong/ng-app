...
!(function (window, angular, undefined) {
  ...
  Date.parseDate = function (str) {
    if (!angular.isString(str) || str.length < 10) return new Date((~~(Date.now() / MS_DAY)) * MS_DAY);
    return new Date(Date.parse(str.substr(0, 10)));
    return new Date(Date.parse(str.substr(0, 10).replace(/-/g, "/")));
  }
  Date.prototype.days = function (otherDate) {
    if (otherDate instanceof Date) return ~~((this - otherDate) / MS_DAY);
    if (angular.isString(otherDate)) return ~~((this - Date.parseDate(otherDate)) / MS_DAY);
    return ~~((+this) / MS_DAY);
  }
