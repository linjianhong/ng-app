...
class CUploadOss {
  ...
  public static function uploadObject($content, $fnOnOSS, $ossName) {
    ...
    return self::uploadObject2rootPath($content, "{$OSS_config['path']}/$fnOnOSS", $ossName);
  }
  public static function uploadObject2rootPath($content, $fnOnOSS, $ossName) {
    $OSS_config = self::getOssConfig($ossName);
    if (!$OSS_config) {
      return \DJApi\API::error(\DJApi\API::E_PARAM_ERROR, "参数错误");
    }
    ...
    try {
      $ossClient->putObject($OSS_config['bucket'], "$fnOnOSS", $content);
    } catch (OssException $e) {
      return \DJApi\API::error(9001, $e->getMessage());
    }

    $path = $OSS_config['baseurl'] . '/';
    return \DJApi\API::OK(["path" => $path, "fn" => $fnOnOSS, "url" => "$path$fnOnOSS"]);
  }
