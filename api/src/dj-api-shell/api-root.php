...
class Configs
{
  ...
  {
    // 保证只引用一次，即使在多个目录中存在
    if (self::$readConfigOnceRecord[$fileName]) {
      return;
    }
...
class API
{
  ...
  {
    return is_array($json) && 0 + $json['errcode'] > 0;
  }
...
function array_find($callback, $arr)
{
  foreach ($arr as $item) {
    if ($callback($item)) {
      return $item;
...
{
  $_NULL = null;
  foreach ($arr as $item) if ($callback($item)) return $item;
  return $_NULL;
}
function &findIndex($arr, $callback)
{
  foreach ($arr as $k=>$item) if ($callback($item)) return $k;
  return -1;
}
...
{
  $R = [];
  foreach ($arr as $item) $R[] = $callback($item);
  return $R;
}
function filter($arr, $callback)
{
  $R = [];
  foreach ($arr as $item) if ($callback($item)) $R[] = $item;
  return $R;
}
